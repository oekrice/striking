...
from data_visualisations import calculate_stats, obtain_striking_markdown, plot_errors_time, plot_bar_charts, plot_histograms, plot_boxes, plot_blue_line, plot_hawkear_line
...
st.set_page_config(page_title="View touches", page_icon="📈")
st.markdown("## View Touches")

st.write(
    """
    This page is for viewing the data from strike times either generated with the Analyse Recording page, from the Touch Library or from an uploaded .csv file. \\
...
if st.session_state.current_touch >= 0:
    ...
    if len(st.session_state.existing_models) > 0:
        ...
        with st.expander("View Grid/Blue Line"):
            ...
            st.session_state.highlight_bells = st.pills("Highlight Bells", st.session_state.options, selection_mode="multi", key = 800 + st.session_state.current_touch)
        ...
        with st.expander("View HawkEar-style error plot"):
            st.empty()
            st.session_state.min_plot_change_hawk, st.session_state.max_plot_change_hawk = st.slider("View changes in range:", min_value = 0, max_value = st.session_state.nrows, value=(st.session_state.start_row, min(300, st.session_state.end_row)), format = "%d", step = 2, key = 1000 + st.session_state.current_touch)
            st.session_state.options = ["Bell %d" % bell for bell in range(1,st.session_state.nbells+1)]
            st.session_state.highlight_bells_hawk = st.pills("Highlight Bells", st.session_state.options, selection_mode="multi")
            plot_hawkear_line(st.session_state.raw_target, st.session_state.raw_actuals, Strike_Data.raw_bells, st.session_state.nbells, st.session_state.lead_length, st.session_state.min_plot_change_hawk, st.session_state.max_plot_change_hawk, st.session_state.highlight_bells_hawk, Strike_Data.correct_bells)
